...
Sloupce tabulky je možné prohodit tlačítkem 🔄`prohodit x ↔ y`, tím dojde k prohození os $(x_1 \leftrightarrow y_1,\ x_2 \leftrightarrow y_2,\ \ldots)$. Tlačítkem 💾`uložit data` dojde k uložení dat tabulky ve formátu `.txt` a tlačítkem 🗑️`smazat data` k jejich odstranění. Jsou-li v tabulce data a uživatel vloží tlačítkem 📂`načíst soubor` nová data, dojde k přepsání starých.
...
Kliknutím na tlačítko 📈`typ regrese` se zobrazí rozbalovací nabídka, ve které je možné vybrat tvar fitované funkce. Po výběru se její obecný předpis vypíše v poli dole pod grafem. Zapnutí a vypnutí zobrazení fitované funkce v grafu se provede tlačítkem 📊`spustit analýzu`. V pravém sloupci se zobrazí parametry fitování a v oblasti dole pod grafem se pod obecným předpisem fitované funkce zobrazí stejná s konkrétními parametry. Tlačítko **95%**`interval spolehlivosti` v grafu zobrazí 95% interval spolehlivosti. Tlačítkem 💾`uložit PNG` dojde k uložení grafu ve formátu PNG.
